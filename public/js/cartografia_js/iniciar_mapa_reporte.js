...
$.ajax({
   ...
   success: function (response) {
       ...
       for (let i = 0; i < capas_tabla.length; i++) {
         ...
         Capas[capas_tabla.length] = new ol.source.ImageWMS({
            url: 'http://' + varglobal + '/geoserver/catastro_gllen/wms',
            params: {
               'FORMAT': 'image/png',
               'VERSION': '1.1.1',
               'tiled': true, 
               'LAYERS': 'catastro_gllen'+':' +'vbusqueda_dinamica',
               'STYLES': null,
               'CQL_FILTER':null
            },
            serverType: 'geoserver'
         ...
         TileWMStotal[capas_tabla.length] = new ol.layer.Image({
               source: Capas[capas_tabla.length]
         });
